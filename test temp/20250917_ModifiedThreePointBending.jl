...
deltamax=range(0,2e-3,10) #mm
    ...
    eq=(1/2)*(Steel_section.w*Steel.E*((Steel_section.t-c)^2-(c)^2)
                +Parylene_section.w*Parylene.E*((Steel_section.t-c+Parylene_section.t)^2-(Steel_section.t-c)^2)
                +Si_section.w*Si.E*((Steel_section.t-c+Parylene_section.t+Si_section.t)^2-(Steel_section.t-c+Parylene_section.t)^2)
                +Steel_section.w*Steel.E*((Steel_section.t-c+Parylene_section.t+Si_section.t+Steel_section.t)^2-(Steel_section.t-c+Parylene_section.t+Si_section.t)^2)
                )
    ...
    for i in deltamax
        ...
        I=(Steel_section.w*(2*Steel_section.t)^3)/12
        ## Krafte und Torque
        F=i*48*Steel.E*I/Steel_section.l^3
        M_max=(F/2)*Steel_section.l/2
        x_p=(Steel_section.l-10e-3)/2
        ...
        m=symbols("m")
        M_steeltop=Steel_section.w*Steel.E*((Steel_section.t-c+Parylene_section.t+Si_section.t+Steel_section.t)^3-(Steel_section.t-c+Parylene_section.t+Si_section.t)^3)
        eqb=M-m*(1/3)*(Steel_section.w*Steel.E*((Steel_section.t-c)^3-(c)^3)+Parylene_section.w*Parylene.E*((Steel_section.t-c+Parylene_section.t)^3-(Steel_section.t-c)^3)+Si_section.w*Si.E*((Steel_section.t-c+Parylene_section.t+Si_section.t)^3-(Steel_section.t-c+Parylene_section.t)^3+M_steeltop)+Steel_section.w*Steel.E*((c)^3))
        solb=solve(eqb,m)
        solb
        m=solb[]
    ...
    Plots.plot!(Fig,deltamax*1000, sigmaSi, label="Si", lw=2, linestyle=:dash, color=:black, marker=:square)
    plot!(Fig,deltamax*1000, sigmaPa_max, label="Parylene max", lw=2, linestyle=:dash, color=:yellow, marker=:circle)
    plot!(Fig,deltamax*1000, sigmaPa_min, label="Parylene min", xlabel="Verschiebungsamplitude(mm)", ylabel="Belastung (MPa)", lw=2, linestyle=:dash, color=:gray, marker=:circle)
    plot!(Fig,deltamax*1000, sigmaSteel, label="Stahl min", xlabel="Verschiebungsamplitude(mm)", ylabel="Belastung (MPa)", lw=2, linestyle=:dash, color=:gray, marker=:circle)
