...
eq1 = us - ((1-v_s)/E_s)*(p*r_m^2)/(r_s^2-r_m^2)*r_m+(((1+v_s)/E_s)*(r_m^2*r_s^2)/r_m)*(p/(r_s^2-r_m^2)) - r_s*alpha_s*deltaT
eq2 = um - ((1-v_m)/E_m)*(p*r_m^2)/(r_s^2-r_m^2)*r_m+(((1+v_m)/E_m)*(r_m^2*r_s^2)/r_m)*(p/(r_s^2-r_m^2)) - r_m*alpha_m*deltaT
eq3 = us - um