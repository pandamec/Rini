...
# Suppose you already solved and have:
# grid :: Ferrite.Grid
# dh   :: Ferrite.DofHandler
# u    :: Vector (displacements, e.g. [ux1, uy1, ux2, uy2, ...])

# extract original coordinates
coords = [Point2f(node.x...) for node in grid.nodes]


# displacement per node (assuming 2 dofs per node)
nnodes = length(coords)
U = [Point2f(u[2i-1], u[2i]) for i in 1:nnodes]

scale = 1  # exaggerate deformation#
deformed = [coords[i] + scale*U[i] for i in 1:nnodes]

# element connectivity
connectivity = [cell.nodes for cell in grid.cells]
...
for conn in connectivity
    poly!(ax, Polygon(collect(getindex.(Ref(coords), conn))),
          color=:gray90, strokewidth=1, strokecolor=:black)
end

# deformed
for conn in connectivity
    poly!(ax2, Polygon(collect(getindex.(Ref(deformed), conn))),
          color=:royalblue, strokewidth=1, strokecolor=:black, transparency=true)
end
